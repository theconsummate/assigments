...
## Read the data
```{r}
comp.ratings <- readLines("data/comp-ratings.txt") %>% lapply(function(l) {parts = unlist(strsplit(l, "\t")); paste(gsub("-n", "", parts[1]), gsub(" ", "\t", parts[2]), sep="\t")}) %>% unlist() %>% textConnection() %>% read.table(sep="\t", header=FALSE, row.names=NULL)

colnames(comp.ratings) <- c("Compound","Word1_mean","Word1_std","Word2_mean","Word2_std","Cpd_mean","Cpd_std","mean1*mean2")

comp.properties <- read.csv("data/comp-properties.txt", sep = "\t", row.names=NULL, na.strings=c("NA", "-", "?", "None"))

co.occ.compounts <- read.table("data/co-occ-compounds1.txt", header=FALSE, sep = "\t", stringsAsFactors=FALSE, row.names=NULL)

co.occ.constituents <- read.table("data/co-occ-constituents1.txt", header=FALSE, sep = "\t", stringsAsFactors=FALSE, row.names=NULL)
```
