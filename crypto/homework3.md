...
## Problem 2
The adversary can keep on selecting plaintexts for as long as it wants but it will never see the same ciphertext again. Because of the PRG, it always generate random output, so the everytime a different cipher is generated.
...
every ciphertext is different, even though the content may or may not be same.

Hence,
Adv_SS = |P(Exp(0)==1)-P(Exp(1)==1)| must be negligble otherwise the system will be insecure.
